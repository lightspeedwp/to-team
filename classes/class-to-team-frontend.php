...
class LSX_TO_Team_Frontend extends LSX_TO_Team{
	...
	public function single_content_bottom() {
		if ( is_singular( 'team' ) ) {
			...
			if ( function_exists( 'lsx_to_gallery' ) ) {
				lsx_to_gallery( '<section id="gallery"><h2 class="section-title">' . esc_html__( 'Gallery', 'lsx-activities' ) . '</h2>', '</section>' );
			} elseif ( class_exists( 'envira_gallery' ) ) {
				lsx_to_envira_gallery( '<section id="gallery"><h2 class="section-title">' . esc_html__( 'Gallery', 'lsx-activities' ) . '</h2>', '</section>' );
			}

			if ( function_exists( 'lsx_to_videos' ) ) {
				lsx_to_videos( '<div id="videos"><h2 class="section-title">' . esc_html__( 'Videos', 'lsx-activities' ) . '</h2>', '</div>' );
			} elseif ( class_exists( 'Envira_Videos' ) ) {
				lsx_to_envira_videos( '<div id="videos"><h2 class="section-title">' . esc_html__( 'Videos', 'lsx-activities' ) . '</h2>', '</div>' );
			}
		}
	}
